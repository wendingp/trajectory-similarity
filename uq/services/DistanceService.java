...
public class DistanceService {
	...
	public void STLCSS_SIGMOID(ArrayList<Trajectory> trajList, ArrayList<Trajectory> tranfTrajList,
		...
		STLCSSWithSigmoidDistanceCalculator STLCSS_SIG = new STLCSSWithSigmoidDistanceCalculator(distanceThreshold,
	...
	// STLCSS Distance
	public void STLCSS(ArrayList<Trajectory> trajList, ArrayList<Trajectory> tranfTrajList, double distanceThreshold,
			long timeIntervalThreshold) {
		STLCSSDistanceCalculator STLCSS = new STLCSSDistanceCalculator(distanceThreshold, timeIntervalThreshold);
		ArrayList<Double> distancesList = new ArrayList<Double>();
		for (int i = 0; i < trajList.size(); i++) {
			ArrayList<Point> r = trajList.get(i).getPointsList();
			ArrayList<Point> s = tranfTrajList.get(i).getPointsList();
	...
	// STLCSS Distance
	public void STLCSS_2(ArrayList<Trajectory> sampleList, ArrayList<Trajectory> testList, double distanceThreshold,
			long timeIntervalThreshold) {
		STLCSSDistanceCalculator STLCSS = new STLCSSDistanceCalculator(distanceThreshold, timeIntervalThreshold);
		ArrayList<Double> distancesList = new ArrayList<Double>();
		for (Trajectory sample : sampleList) {
			for (Trajectory test : testList) {
				// ArrayList<Point> r = ;
				// ArrayList<Point> s = ;
				distancesList.add(STLCSS.getDistance(sample.getPointsList(), test.getPointsList()));
			}
		}
		String d = distanceThreshold == 0.01 ? "01" : "0004";
		FileService.saveDistanceFile(distancesList, "STLCSS_" + d + TRANSF_NAME);
	...
	public void Euclidean(ArrayList<Trajectory> trajList, ArrayList<Trajectory> tranfTrajList) {
		EuclideanDistanceForMultiDimensionTimeSeriesCalculator EUCLIDEAN = new EuclideanDistanceForMultiDimensionTimeSeriesCalculator();
		ArrayList<Double> distancesList = new ArrayList<Double>();
		for (int i = 0; i < trajList.size(); i++) {
			ArrayList<Point> r = trajList.get(i).getPointsList();
			ArrayList<Point> s = tranfTrajList.get(i).getPointsList();
	...
	/**
	 * Run all the distance functions for the given trajectories lists. Distances
	 * between every trajectory and its transformation. Calculates the distance
	 * between every trajectory in the trajectories list with its transformation in
	 * the transformation list. Save the outcomes to files.
	 */
	public void runDistanceFunctions(final ArrayList<Trajectory> trajList, final ArrayList<Trajectory> transfTrajList) {
	...
	/**
	 * Run all the distance functions for the given trajectories lists. Distances
	 * between sample and test trajectories. Calculates the distance between every
	 * trajectory in the sample list with every trajectory in the test list. Save
	 * the outcomes to files.
	 */
	public void runDistanceFunctions_2(ArrayList<Trajectory> sampleList, ArrayList<Trajectory> testList) {
