...
public class Main {
    ...
    private static final TransformationService transfService = new TransformationService();

    // Get instance of the distance functions
    private static final DistanceService distService = new DistanceService();
    ...
    /**
     * Read the distance files and calculate the distances mean and standard
     * deviation. Save the results into a single file if required (true). Each
     * output file line contains three columns: the distance file name | the mean |
     * and standard deviation.
     *
     * @param save
     *                 Whether or not to save the output file
     */
    public static void calculateDistancesMeanAndStd(boolean save) {
        ArrayList<DistanceFile> distanceFiles = FileService.readDistances(false);
        ...
        for (DistanceFile file : distanceFiles) {
            ...
            fileScript.append(file.fileName).append(" ").append(mean).append(" ").append(std).append("\n");
        ...
        if (save) {
            FileService.saveDistancesAnalysisFile(fileScript.toString(), fileName);
        }
    }

    /**
     * Calculate the distances BEFORE transformation. Compare sample trajectories
     * with test trajectories. Run the distance functions and save the files to the
     * output folder. These are the 'ground truth' files for every distance
     * function.
     */
    public static void calculaDistancesBeforeTransformation() {
        // read sample and test trajectories from folders
        ArrayList<Trajectory> sampleList = FileService.readSampleTrajectories(); // read from sample folder
        ArrayList<Trajectory> testList = FileService.readTestTrajectories(); // read from test folder
    ...
    /**
     * Calculate the distances AFTER transformation. Compare sample trajectories
     * with test trajectories for every transformation and parameter. Run the
     * distance functions and save the files to the output folder.
     */
    public static void calculaDistancesAfterTransformation() {
        // read sample and test trajectories from folders
        ArrayList<Trajectory> sampleList = FileService.readSampleTrajectories();
        ArrayList<Trajectory> testList = FileService.readTestTrajectories();
        ...
        for (double addRateParam : addRate) {
            for (double noiseDistParam : noiseDist) {
                ...
                // calculate distances to transformed trajectories
                distService.TRANSF_NAME = "_add_noise_" + (int) (addRateParam * 100) + "pct_"
                        + (int) (noiseDistParam * 100) + "d";
                distService.runDistanceFunctions_2(sampleList, newTestList);
            }
        }
    }
    ...
    /**
     * Calculate the distances between every trajectory and its transformation. Read
     * original trajectories, run the transformations and compare the transformed
     * trajectory with the original. Save the results to files.
     */
    public static void calculateDistancesForEveryTrajectory() {
        ...
        for (double addRate : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.addPointsTransformation(trajList, addRate);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_add_pts_" + (int) (addRate * 100) + "pct";
        ...
        for (double deleteRate : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.deletePointsTransformation(trajList, deleteRate);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_delete_pts_" + (int) (deleteRate * 100) + "pct";
            distService.runDistanceFunctions(trajList, transfTrajList);
        }
        ...
        for (double sampleRate : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.diffSampleRateTransformation(trajList,
                    (int) sampleRate);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_dif_time_rate_" + (int) (sampleRate) + "pts";
            distService.runDistanceFunctions(trajList, transfTrajList);
        ...
        for (double scaleRatio : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.scaleTransformation(trajList, scaleRatio);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_scale_" + (int) (scaleRatio * 100) + "pct";
            distService.runDistanceFunctions(trajList, transfTrajList);
        }
        ...
        for (double timeRatio : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.timeScaleTransformation(trajList, timeRatio);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_time_scale_" + (int) (timeRatio * 100) + "pct";
            distService.runDistanceFunctions(trajList, transfTrajList);
        }
        ...
        for (double angle : parameters) {
            ...
            ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
            // Get the transformation
            ArrayList<Trajectory> transfTrajList = transfService.rotationTransformation(trajList, angle);
            // calculate distances to transformed trajectories
            distService.TRANSF_NAME = "_rotation_" + (int) Math.round(Math.toDegrees(angle)) + "dgr";
            distService.runDistanceFunctions(trajList, transfTrajList);
        }
        ...
        for (double addRateParam : addRate) {
            for (double noiseDistParam : noiseDist) {
                ...
                ArrayList<Trajectory> trajList = FileService.readAllTrajectories();
                // Get the transformation
                ArrayList<Trajectory> transfTrajList = transfService.addNoiseTransformation(trajList, addRateParam,
                        noiseDistParam);
                // calculate distances to transformed trajectories
                distService.TRANSF_NAME = "_add_noise_" + (int) (addRateParam * 100) + "pct_"
                        + (int) (noiseDistParam * 100) + "d";
     ...
     * Must read all files from the same distance function for a given
     * transformation function regardless the parameter.
     *
     * @param save
     *                 Whether to save the outcomes to files. Files name same as the
     *                 original file + _norm. Save to the Output folder.
     */
    public static void normalizeDistances(boolean save) {
        // read the files
        // files must be from the same distance measure
        ArrayList<DistanceFile> distanceFilesList = FileService.readDistances(false);
        ...
        for (DistanceFile distFile : distanceFilesList) {
            ...
            DistanceFile normalizedDistFile = new DistanceFile(distFile.fileName + "_norm");
            ...
            if (save) {
                FileService.saveDistanceFile(normalizedDistFile);
            }
    ...
    /**
     * Compare the distances values for a certain distance function and print the
     * variation outcomes.
     */
    public static void runDistanceAnalysis() {
        ArrayList<DistanceFile> distanceFiles = FileService.readDistances(false);
        DistanceFile groundTruthFile = FileService.readDistances(true).get(0);

        System.out.println("ANALYSIS RESULTS: \n");
        for (DistanceFile file : distanceFiles) {
            // for each distance file get the distances variation from the ground truth
            ArrayList<Double> distVariationList = compareDistances(groundTruthFile.getDistancesList(),
                    file.getDistancesList());
    ...
    /**
     * Read the original trajectory files from input folder, shift the start time to
     * 0 and save them to new files.
     */
    public static void reSampleTrajectories() {
        ArrayList<Trajectory> trajectoryList;
        ArrayList<Trajectory> newTrajList = new ArrayList<>();
        ...
        TimeShiftTransformation timeShiftTransf = new TimeShiftTransformation(0);
    ...
    /**
     * Select a random number of trajectories from the input folder and save them to
     * disc.
     */
    public static void selectAndSaveRandomTrajectories(int numTrajectories) {
        ...
        ArrayList<Trajectory> randomTraj = FileService.selectRandomTrajectories(sampleList, numTrajectories);

        FileService.saveTrajectoriesFiles(randomTraj, numTrajectories);
    }

    /**
     * compare a list of distance with another (ground truth). Return a list of the
     * variation for every distance.
     */
    private static ArrayList<Double> compareDistances(ArrayList<Double> groundTruthList,
                                                      ArrayList<Double> distanceList) {
        ArrayList<Double> variationList = new ArrayList<>();
